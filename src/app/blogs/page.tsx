    ...
    <>
      <section className="w-full py-12 bg-transparent">
        ...
        >
          ...
          >
            List of my all Blogs with Medium link.
          </motion.p>
      ...
      <AnimatePresence>
        {active && typeof active === "object" && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 mt-12 bg-black/20 h-full w-full z-10"
          />
        )}
      </AnimatePresence>
      <AnimatePresence>
      ...
      <ul className="max-w-[800px] mt-8 mx-auto w-full grid grid-cols-1 md:grid-cols-2 items-start gap-4">
        {Cards.map((card, index) => (
          <motion.div
            layoutId={`card-${card.title}-${id}`}
            key={card.title}
            onClick={() => setActive(card)}
            className="p-4 border mx-4 md:mx-2 flex flex-col  hover:bg-neutral-100 dark:hover:bg-neutral-800 rounded-xl cursor-pointer"
          >
